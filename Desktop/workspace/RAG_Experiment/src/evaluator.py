...
Metrics computed:
  ...
  - source_diversity  : fraction of unique PDF sources among retrieved chunks
...
def compute_source_diversity(retrieved_chunks: List[Dict[str, Any]]) -> float:
    """Fraction of unique document sources among retrieved chunks (0–1)."""
    if not retrieved_chunks:
        return 0.0
    sources = set()
    for chunk in retrieved_chunks:
        meta = chunk.get("metadata") or {}
        sources.add(meta.get("source") or chunk.get("text", "")[:40])
    return float(len(sources) / len(retrieved_chunks))
...
def evaluate_single(result: Dict[str, Any], query_vector: np.ndarray, embed_fn) -> Dict[str, Any]:
    ...
    result["source_diversity"] = compute_source_diversity(retrieved)
    return result
...
def summarise_results(all_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for key, results in groups.items():
        ...
        logger.debug(f"Summarised {key}: n={len(results)}")
    return summary
